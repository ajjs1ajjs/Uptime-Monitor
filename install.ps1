...
$isUpdate = (Get-Service -Name $ServiceName -ErrorAction SilentlyContinue) -or (Test-Path $Binary) -or (Test-Path "$DataDir\sites.db")
...
# verify the downloaded file is a runnable executable
& $Binary --version *> $null
...
# auto-restart on failure (equivalent of Restart=always in systemd unit)
...
# --- Admin password (fresh install only, unless provided) ---------------------------
$adminSet = $false
...
if ($adminSet) {
    ...
    Write-Host "===================================="
    Write-Host "При вході система попросить змінити пароль. Запишіть його."
} else {
