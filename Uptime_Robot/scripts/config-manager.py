    ...
    try:
        ...
        for key in set(old_config.keys()) | set(new_config.keys()):
            if old_config.get(key) != new_config.get(key):
                changes[key] = {"old": old_config.get(key), "new": new_config.get(key)}
            ...
            if os.path.exists(backup_script):
                ...
                subprocess.Popen(
                    [
                        ...
                        "on-change",
                        "--dest",
                        backup_config.get("backup_dir", "/backup/uptime-monitor"),
                        "--comment",
                        comment,
                        "--quiet",
                    ],
...
def main():
    ...
    if command == "backup":
        ...
        if backups:
            ...
            print("")
            print(
                "Latest:  config.latest.json ->",
                (
                    os.readlink(os.path.join(get_backup_dir(), "config.latest.json"))
                    if os.path.islink(os.path.join(get_backup_dir(), "config.latest.json"))
                    else "N/A"
                ),
            )
            print(
                "Previous: config.previous.json ->",
                (
                    os.readlink(os.path.join(get_backup_dir(), "config.previous.json"))
                    if os.path.islink(os.path.join(get_backup_dir(), "config.previous.json"))
                    else "N/A"
                ),
            )
        else:
