...
def get_alert_policy() -> dict[str, Any]:
    ...
    result["still_down_repeat_seconds"] = _clamp(result.get("still_down_repeat_seconds"), 60, 86400, 600)
    result["ssl_notification_cooldown_seconds"] = _clamp(result.get("ssl_notification_cooldown_seconds"), 300, 86400, 21600)
    ...
    result["flapping_suppression_seconds"] = _clamp(result.get("flapping_suppression_seconds"), 60, 7200, 600)
    ...
    result["ssl_notification_days"] = sorted(set(int(d) for d in ssl_days if d > 0), reverse=True)
