...
async def is_under_maintenance(site_id: int) -> bool:
    ...
    # Daily/weekly windows are entered as wall-clock LOCAL time (there is no tz
    # field on the window), so they must be evaluated against local time, not
    # UTC — otherwise a "02:00" window fires at 02:00 UTC, off by the offset.
    ...
    for row in rows:
        ...
        if rule_type == "one_off":
            ...
            try:
                ...
                if today_start <= local_time <= today_end:
                    return True
                ...
                if yesterday_start <= local_time <= yesterday_end:
                    return True
            except Exception:
                pass
