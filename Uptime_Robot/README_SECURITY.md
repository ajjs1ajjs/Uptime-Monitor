...
## 🛡️ Hardening-пас (2.1.x)

Додаткові заходи безпеки після аудиту:

| Захист | Опис |
|--------|------|
| **CSRF** | Middleware тепер **зареєстровано**: для `/api/*` — перевірка `Origin`/`Referer` (same-origin), для форм `change-password`/`forgot-password` — одноразові токени. |
| **SSRF** | Валідація URL моніторингу резолвить хост і блокує приватні / loopback / link-local (`169.254.169.254`) / reserved / multicast адреси, а не лише літеральні приватні IP. |
| **Паролі поза логами** | Пароль адміна показується **один раз** у stdout при створенні; не пишеться в журнал і не друкується повторно при рестарті. Відновлення — `auth_cli show-password`. |
| **Політика паролів** | Скидання пароля через `PUT /api/users/{username}` застосовує ті самі правила стійкості, що й створення. |
| **Reverse-proxy** | `UPTIME_MONITOR_TRUSTED_PROXIES` (список IP проксі через кому) дозволяє довіряти `X-Forwarded-For` для коректного rate-limiting за реальним IP. За замовчуванням вимкнено. |

### Змінні оточення, пов'язані з безпекою

| Змінна | Призначення |
|--------|-------------|
| `UPTIME_MONITOR_MASTER_KEY` | Майстер-ключ Fernet для шифрування секретів (інакше читається з `master.key`). |
| `UPTIME_MONITOR_TRUSTED_PROXIES` | Кома-розділений список IP довірених reverse-proxy для `X-Forwarded-For`. |
| `UPTIME_MONITOR_ADMIN_PASSWORD` | Задати пароль адміна замість випадкового (для автоматизованого розгортання/тестів). |
...
**Стандартний логін:**
- Username: `admin`
- Password: випадковий (генерується при встановленні; показується один раз у stdout і зберігається в `credentials.txt`. Для перегляду пізніше: `python -m Uptime_Robot.auth_cli show-password` — у журнал **не** пишеться)
